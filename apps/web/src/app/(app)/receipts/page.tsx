...
export default function ReceiptsListPage() {
    ...
    const columnOptions = [
        ...
        { key: "voucherNo", label: "Receipt No", defaultVisible: true },
        { key: "particulars", label: "Payer / Particulars", defaultVisible: true },
    ...
    const metrics = React.useMemo(() => {
        const totalAmount = filteredVouchers.reduce((acc, v) => {
            // Receipt amount is usually the total of lines or a specific field
            // In listVouchers, it seems it's pre-calculated or we use total credit
            const amount = v.amount || v.lines?.reduce((s: number, l: any) => s + Number(l.credit || 0), 0) || 0;
        ...
        <div className="space-y-6 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <PageHeader
                    ...
                    description="Monitor and manage all incoming payments and customer collections."
                    actions={
                        <Button
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-slate-900/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between">
                    <div className="flex flex-col">
                        <span className="text-[10px] uppercase font-black tracking-widest text-slate-400">Total Collections</span>
                ...
                <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 text-white p-5 rounded-2xl shadow-lg shadow-emerald-500/20 flex flex-col justify-center">
                    <span className="text-[10px] uppercase font-black tracking-widest text-emerald-200">System Status</span>
                    <div className="flex items-center gap-2 mt-2">
                        <div className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                        <span className="font-bold text-sm">Collections Synced</span>
                    </div>
                </div>
            </div>
            ...
            <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-xl shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-950 dark:shadow-none">
                {loading && filteredVouchers.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-24 space-y-4">
                        ...
                        <p className="text-sm font-medium text-slate-500 animate-pulse uppercase tracking-widest text-[10px] font-black">Syncing Collections...</p>
                    </div>
                        ...
                        <table className="w-full text-sm text-left">
                            <thead>
                                <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/30">
                                    ...
                                    {visibleColumns.includes("voucherNo") && <th className={cn("px-6 font-black text-slate-400 uppercase tracking-widest text-[10px]", compactMode ? "py-3" : "py-4")}>Receipt No</th>}
                                    {visibleColumns.includes("particulars") && <th className={cn("px-6 font-black text-slate-400 uppercase tracking-widest text-[10px]", compactMode ? "py-3" : "py-4")}>Payer / Particulars</th>}
                            ...
                            <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                                {filteredVouchers.map((v, index) => {
                                    ...
                                    const totalAmount = v.amount || v.lines?.reduce((sum: number, line: any) => sum + Number(line.credit || 0), 0) || 0;

                                    // Extract payer name or ledger accounts
                                    const firstAccount = v.party?.name || v.lines?.[0]?.account?.name || v.voucherNumber || v.voucherNo || `RCPT-${v.id.slice(0, 6)}`;
                                    ...
                                    return (
                                        <React.Fragment key={v.id}>
                                            <tr className="group hover:bg-emerald-50/20 dark:hover:bg-emerald-900/10 transition-colors">
                                                ...
                                                {visibleColumns.includes("voucherNo") && (
                                                    <td className={`px-6 ${py} whitespace-nowrap`}>
                                                        <span
                                                            onClick={() => router.push(`/receipts/create?id=${v.id}`)}
                                                            className="font-black text-slate-900 dark:text-white hover:text-emerald-600 transition-colors cursor-pointer"
                                                        >
                                                ...
                                                {visibleColumns.includes("particulars") && (
                                                    ...
                                                    >
                                                        <div className="flex flex-col max-w-[250px]">
                                                            <span className="truncate font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 transition-colors" title={firstAccount}>
                                                                {firstAccount}
                                                            </span>
                                                            {totalEntries > 0 && (
                                                ...
                                                <td className={`px-6 ${py} text-right`}>
                                                    <div className="text-slate-300 group-hover:text-emerald-400 transition-all">
                                                        {isExpanded ? (
                                                ...
                                                <tr className="bg-slate-50/50 dark:bg-slate-900/20">
                                                    <td colSpan={visibleColumns.length + 1} className="px-6 py-4">
                                                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
                                                            <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">Receipt Breakdown</h4>
                    ...
                    <div className="flex flex-col items-center justify-center py-32 px-6 text-center space-y-4">
                        ...
                        <div className="max-w-xs space-y-1">
                            ...
                            <p className="text-sm text-slate-500 font-medium leading-relaxed">Adjust your filters or record your first receipt to start tracking incoming payments.</p>
                        </div>
                        <Button
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 p-4 bg-slate-50/50 dark:bg-slate-900/30 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold px-2 py-1 outline-none focus:ring-2 ring-emerald-500/50"
                        >
