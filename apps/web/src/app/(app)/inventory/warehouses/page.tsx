...
export default function WarehousesPage() {
  ...
  const handleAddBin = async (warehouseId: string) => {
    ...
    try {
      ...
      setSuccess("Bin added");
      setAddBinWarehouseId(null);
      setBinName("");
      setBinCode("");
      await refresh();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6 pb-20 text-foreground">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <PageHeader
            ...
            description="Manage storage locations and bins for your inventory."
            icon={Warehouse}
          />
        </div>
      ...
      {success && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 dark:bg-emerald-900/20 dark:border-emerald-800/30 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-300">
          {success}
        </div>
      )}
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={creating}
                  className="rounded-2xl h-10 bg-blue-600 hover:bg-blue-700 text-white flex-1"
                >
                ...
                <Button
                  variant="ghost"
                  onClick={() => { setCreateOpen(false); setCreateName(""); setCreateCode(""); }}
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          warehouses.map((wh) => (
            <Card key={wh.id} className="border-border/50 shadow-md overflow-hidden transition-shadow hover:shadow-lg">
              <CardContent className="pt-5 pb-5">
                {/* Warehouse row */}
                    ...
                    {editId === wh.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          placeholder="Code"
                          className="rounded-lg h-9 text-sm max-w-[120px]"
                        />
                        <Button size="sm" onClick={() => handleUpdate(wh.id)} className="h-9 rounded-lg bg-blue-600 text-white text-xs">
                          Save
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => setEditId(null)} className="h-9 rounded-lg text-xs">
                          Cancel
                        </Button>
                      </div>
                    ) : (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {!wh.isActive && (
                            <span className="text-[10px] uppercase font-bold text-red-500 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded-full">
                              Inactive
                            </span>
                          )}
                        </div>
                  ...
                  <div className="mt-4 ml-12 pl-4 border-l-2 border-blue-100 dark:border-blue-900/50">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          setAddBinWarehouseId(wh.id);
                          setBinName("");
                          setBinCode("");
                        }}
                        className="rounded-xl h-8 text-xs"
                      >
                        <Plus className="mr-1 h-3 w-3" /> Add Bin
                      </Button>
                    </div>
                    ...
                    {addBinWarehouseId === wh.id && (
                      <div className="flex items-center gap-2 mb-3 p-3 rounded-xl bg-muted/50 border border-border/50">
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => setAddBinWarehouseId(null)}
                    ...
                    {wh.bins.length === 0 ? (
                      <p className="text-xs text-muted-foreground py-4 text-center">
                        No bins defined. Bins help you track stock within specific shelf locations.
                      </p>
                    ) : (
                      <div className="space-y-1">
                        {wh.bins.map((bin) => (
