...
let InventoryService = class InventoryService {
    ...
    async adjustStock(user, input) {
        ...
        const needsApproval = Boolean(settings.adjustmentApprovalRequired) ||
    ...
    async postApprovedStockAdjustment(user, input) {
        const item = await this.prisma.item.findFirst({
    ...
    async postApprovedStockTransfer(user, input) {
        const item = await this.prisma.item.findFirst({
            where: { id: input.itemId, companyId: user.companyId }
        });
        if (!item)
    ...
    async approveMovementApproval(user, id, input) {
        ...
        const approvalPayload = approval.payloadJson;
    ...
    async executeApprovedMovementReversal(user, originalApprovalId, reason) {
        const db = this.prisma;
        const original = await db.inventoryMovementApproval.findFirst({ where: { id: originalApprovalId, companyId: user.companyId } });
        if (!original)
            throw new common_1.BadRequestException("Original approved movement not found");
        if (original.status !== "approved")
            throw new common_1.BadRequestException("Only approved movement requests can be reversed");
        return this.markMovementApprovalReversed(user, original, reason);
    }
    async markMovementApprovalReversed(user, approval, reason) {
        const db = this.prisma;
