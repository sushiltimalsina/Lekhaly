...
export class InventoryService {
  ...
  async listMovementApprovals(user: AuthUser, query: { status?: string; movementType?: string; take?: number }) {
    ...
    for (const row of approvals) {
      ...
      if (payload.itemId) itemIds.add(payload.itemId);
      for (const key of ["warehouseId", "fromWarehouseId", "toWarehouseId"]) if (payload[key]) warehouseIds.add(payload[key]);
      for (const key of ["binId", "fromBinId", "toBinId"]) if (payload[key]) binIds.add(payload[key]);
    }
