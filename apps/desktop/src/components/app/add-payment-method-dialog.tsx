...
import { createPaymentMethod, updatePaymentMethod } from "@/lib/api/payment-methods";
...
export default function AddPaymentMethodDialog({ open, onClose, onSuccess, initialData }: AddPaymentMethodDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let res;
            if (initialData?.id) {
                res = await updatePaymentMethod(initialData.id, {
                    name: name.trim(),
                    isActive
                });
            } else {
            ...
            <div className="w-full max-w-md rounded-[2.5rem] border bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950 overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
                <div className="flex items-center justify-between border-b px-8 py-5 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
                    <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-2xl bg-emerald-600 flex items-center justify-center text-white shadow-lg shadow-emerald-200 dark:shadow-none">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                                {initialData ? "Update existing payment method details" : "Define a new way to receive payments"}
                            </p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    <label className="space-y-1.5 block">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">METHOD NAME *</span>
                        <Input
                            autoFocus
                            value={name}
                            onChange={e => setName(e.target.value)}
                            placeholder="e.g. PayPal, Stripe, Mobile Wallet"
                            className="h-12 rounded-2xl bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-emerald-500"
                        />
                    </label>
                    ...
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 dark:bg-slate-900/30 border border-slate-100 dark:border-slate-800">
                        <div className="space-y-0.5">
                            ...
                            <div className="text-[10px] text-muted-foreground">Enable or disable this payment method</div>
                        </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t dark:border-slate-800">
                        ...
                        <button type="submit" disabled={saving} className="h-12 px-6 rounded-2xl text-sm font-bold bg-emerald-600 text-white hover:bg-emerald-700 shadow-xl shadow-emerald-200 transition-all flex items-center gap-2 font-mono">
