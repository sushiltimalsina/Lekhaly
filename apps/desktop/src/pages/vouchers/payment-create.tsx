// apps/desktop/src/pages/vouchers/payment-create.tsx
...
export default function PaymentCreatePage() {
        ...
        const load = async () => {
          try {
            const [a, p, v] = await Promise.all([
              listAccounts(),
              listParties(),
              listVouchers({ type: "payment", take: 5 })
            ]);
            setAccounts(a || []);
            setParties(p || []);
    ...
    const onSave = async () => {
        if (!totals.balanced) {
            setError("Payment must be balanced.");
            return;
        }
        ...
        <div className="space-y-6 animate-fade-in pb-20">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    ...
                    <Button onClick={onSave} disabled={loading || !totals.balanced} className="h-11 px-8 rounded-2xl bg-rose-600 text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-rose-100 italic">
                        <Save className="mr-2 h-4 w-4" /> Commit Payment
                    </Button>
                </div>
            </div>

            <PageHeader 
                title="Cash/Bank Payment Entry" 
                description="Record outgoing funds to suppliers, expenses, or other entities." 
            />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="p-8 bg-white rounded-[32px] border-2 border-slate-50 shadow-sm flex flex-wrap gap-12">
                        ...
                        <div className="flex-1 min-w-[200px] space-y-2">
                             <label className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Reference / Instrument No.</label>
                            <Input
                                value={form.referenceNo}
                                onChange={(e) => setForm(f => ({ ...f, referenceNo: e.target.value }))}
                                placeholder="Cheque / TRF / Internal Ref"
                    ...
                    <div className="bg-white rounded-[32px] border-2 border-slate-50 shadow-sm overflow-hidden">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {lines.map((line, idx) => (
                                    <tr key={line.id} className="group hover:bg-rose-50/10 active:bg-slate-50/50 transition-colors">
                        ...
                        <div className="p-6 bg-slate-50/30 border-t border-slate-100">
                             <Button variant="outline" onClick={addLine} className="h-12 w-full rounded-2xl border-dashed border-rose-200 text-[10px] font-black uppercase tracking-[0.2em] text-rose-600 hover:bg-rose-50">
                ...
                <div className="space-y-8">
                    ...
                    <div className="p-8 bg-slate-900 rounded-[32px] text-white shadow-2xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-8 opacity-10"><CreditCard className="h-24 w-24" /></div>
                    ...
                    <div className="p-8 bg-white rounded-[32px] border-2 border-slate-50 shadow-sm space-y-4">
                        ...
                        <textarea
                            ...
                            className="min-h-[140px] w-full rounded-2xl border-2 border-slate-50 bg-slate-50/50 p-5 text-sm outline-none focus:border-rose-500 focus:bg-white transition-all font-medium leading-relaxed resize-none"
                        />
                    </div>
                </div>
            </div>
                    ...
                    <div className="w-[400px] h-full bg-white shadow-2xl p-8 animate-in slide-in-from-right duration-300">
                        <div className="flex items-center justify-between mb-8 pb-4 border-b">
                            <h3 className="text-sm font-black uppercase tracking-widest text-slate-600 italic flex items-center gap-3">
                                <History className="h-5 w-5 text-rose-600" /> Recent Payments
                            </h3>
                        ...
                        <div className="space-y-4">
                            {recentVouchers.map(v => (
                                <div key={v.id} onClick={() => { navigate(`/vouchers/view/${v.id}`); setShowHistory(false); }} className="p-5 rounded-3xl border border-slate-100 hover:border-rose-500 hover:shadow-lg transition-all cursor-pointer group">
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="text-[11px] font-black text-slate-900 uppercase">#{v.voucherNo}</span>
                                        <span className="text-sm font-black group-hover:text-rose-600"><MoneyText value={v.amount} /></span>
                                    </div>
