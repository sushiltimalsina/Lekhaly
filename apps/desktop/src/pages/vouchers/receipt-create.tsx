// apps/desktop/src/pages/vouchers/receipt-create.tsx
        ...
        const load = async () => {
          try {
            const [a, p, v] = await Promise.all([
              listAccounts(),
              listParties(),
              listVouchers({ type: "receipt", take: 5 })
            ]);
            setAccounts(a || []);
            setParties(p || []);
        ...
        <div className="space-y-6 animate-fade-in pb-20">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    ...
                    <Button onClick={onSave} disabled={loading || !totals.balanced} className="h-11 px-8 rounded-2xl bg-emerald-600 text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-emerald-100 italic">
                        <Save className="mr-2 h-4 w-4" /> Commit Receipt
                    </Button>
                </div>
            </div>

            <PageHeader 
                title="Money Receipt Entry" 
                description="Register incoming funds and balance your liquid accounts." 
            />
                    ...
                    <div className="bg-white rounded-[32px] border-2 border-slate-50 shadow-sm overflow-hidden">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {lines.map((line, idx) => (
                                    <tr key={line.id} className="group hover:bg-emerald-50/10 active:bg-slate-50/50 transition-colors">
                        ...
                        <div className="p-6 bg-slate-50/30 border-t border-slate-100">
                             <Button variant="outline" onClick={addLine} className="h-12 w-full rounded-2xl border-dashed border-emerald-200 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 hover:bg-emerald-50">
                ...
                <div className="space-y-8">
                    ...
                    <div className="p-8 bg-white rounded-[32px] border-2 border-slate-50 shadow-sm space-y-4">
                        ...
                        <textarea
                            ...
                            className="min-h-[140px] w-full rounded-2xl border-2 border-slate-50 bg-slate-50/50 p-5 text-sm outline-none focus:border-emerald-500 focus:bg-white transition-all font-medium leading-relaxed resize-none"
                        />
                    </div>
                </div>
            </div>
                    ...
                    <div className="w-[400px] h-full bg-white shadow-2xl p-8 animate-in slide-in-from-right duration-300">
                        <div className="flex items-center justify-between mb-8 pb-4 border-b">
                            <h3 className="text-sm font-black uppercase tracking-widest text-slate-600 italic flex items-center gap-3">
                                <History className="h-5 w-5 text-emerald-600" /> Recent Receipts
                            </h3>
                        ...
                        <div className="space-y-4">
                            {recentVouchers.map(v => (
                                <div key={v.id} onClick={() => { navigate(`/vouchers/view/${v.id}`); setShowHistory(false); }} className="p-5 rounded-3xl border border-slate-100 hover:border-emerald-500 hover:shadow-lg transition-all cursor-pointer group">
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="text-[11px] font-black text-slate-900 uppercase">#{v.voucherNo}</span>
                                        <span className="text-sm font-black group-hover:text-emerald-600"><MoneyText value={v.amount} /></span>
                                    </div>
