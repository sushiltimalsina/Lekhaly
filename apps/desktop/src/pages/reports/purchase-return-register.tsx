...
export default function PurchaseReturnRegisterPage() {
    ...
    async function run() {
        ...
        try {
            ...
            setError(e?.message ?? "Failed to load purchase return register");
            setRows([]);
        } finally {
            setLoading(false);
        }
    ...
    const columns: Column<VoucherRecord>[] = [
        ...
        { key: "voucherNo", header: "DN No", width: 140, cell: (r) => <span className="mono-numbers font-bold">{r.voucherNo}</span> },
        { key: "memo", header: "Supplier / Detail", cell: (r) => <div className="text-sm truncate max-w-sm">{r.memo ?? "â€”"}</div> },
        { key: "status", header: "Status", width: 120, cell: (r) => <StatusBadge status={r.status as DocStatus} /> },
        {
            key: "amount",
            header: <span className="w-full block text-right font-black">Return Amount</span>,
            align: "right",
            width: 160,
            cell: (r) => <MoneyText value={Number(r.amount ?? 0)} className="font-bold text-red-600 dark:text-red-400" />
        }
    ];
    ...
    return (
        <div className="space-y-6 pb-20 text-foreground">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between print:hidden">
                <PageHeader title="Purchase Return Register" description="Log of all supplier returns and debit notes issued." />
                ...
                <Card className="border-border/50 bg-red-600/5 shadow-none overflow-hidden relative text-foreground">
                    <CardContent className="pt-6">
                        <div className="text-[10px] uppercase font-bold tracking-widest text-red-600 flex items-center gap-2">
                            <RotateCw className="h-3 w-3" /> Total Purchase Return
                        </div>
            ...
            <Card className="border-border/50 glass-card overflow-hidden shadow-xl min-h-[400px]">
                <DataTable rows={rows} columns={columns} loading={loading} emptyText="No purchase returns recorded" className="border-none" />
                {rows.length > 0 && (
                    <div className="flex items-center justify-end border-t border-border/50 bg-red-50/20 px-6 py-6 font-black">
