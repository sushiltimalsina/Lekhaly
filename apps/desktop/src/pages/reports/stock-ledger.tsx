...
function getSourceHref(row: StockLedgerEntry) {
    if (row.sourceDocumentType === "invoice" && row.sourceDocumentId) {
        ...
        return `/sales/view/${row.sourceDocumentId}`;
    }
    ...
    const itemLedgerColumns: Column<StockLedgerEntry>[] = [
        { key: "date", header: "Date", width: 130, cell: (r) => <div className="text-xs font-medium">{new Date(r.date).toLocaleDateString()}</div> },
        {
            ...
            cell: (r) => getSourceHref(r) ? (
                <Link to={getSourceHref(r)!} className="inline-flex flex-col font-semibold text-primary hover:underline">
    ...
    const columns: Column<StockReportRow>[] = [
        {
            key: "name", header: "Item / SKU", width: 260, cell: (r) => (
                <div className="flex flex-col">
                    <Link to={`/items/view/${r.id}?tab=ledger&from=stock-ledger`} className="font-bold text-foreground hover:text-primary hover:underline">{r.name}</Link>
        ...
        <div className="space-y-6 pb-20 text-foreground">
            ...
            <Card className="border-border/50 glass-card overflow-hidden shadow-xl min-h-[400px]">
                <DataTable
                    ...
                    className="border-none"
                    onRowClick={(row) => navigate(`/items/view/${row.id}?tab=ledger&from=stock-ledger`)}
                />
            </Card>
                </>
            )}
