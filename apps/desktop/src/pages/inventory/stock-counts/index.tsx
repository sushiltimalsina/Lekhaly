  ...
  return (
    <div className="space-y-6 pb-20 text-foreground animate-fade-in">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <PageHeader title="Physical Stock Count" description="Conduct physical inventory counts and generate variance adjustments." icon={ClipboardList} />
      ...
      <div className="grid gap-4">
        {loading ? (
          ...
          counts.map((count) => (
            <Card key={count.id} className="border-border/50 shadow-md hover:shadow-lg transition-shadow cursor-pointer group" onClick={() => navigate(`/inventory/stock-counts/view/${count.id}`)}>
              <CardContent className="pt-5 pb-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-orange-500/10 text-orange-600 dark:text-orange-400 shrink-0"><ClipboardList className="h-6 w-6" /></div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-foreground text-lg">{count.reference || "Untitled Count"}</span>
                      <span className={cn("text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border", count.status === "completed" ? "bg-emerald-50 text-emerald-600 border-emerald-200" : count.status === "in_progress" ? "bg-blue-50 text-blue-600 border-blue-200" : "bg-slate-50 text-slate-600 border-slate-200")}>{count.status.replace("_", " ")}</span>
                    </div>
                    <div className="text-xs text-muted-foreground mt-1 flex items-center gap-3">
                      <span>Date: {new Date(count.countDate).toLocaleDateString()}</span>
                      {count.warehouse && <span className="flex items-center gap-1">• <Package className="h-3 w-3" /> {count.warehouse.name}</span>}
