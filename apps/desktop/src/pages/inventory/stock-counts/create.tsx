    ...
    <div className="space-y-6 pb-20 text-foreground max-w-5xl mx-auto animate-fade-in">
      ...
      <Card className="border-border/50 shadow-lg"><CardContent className="pt-6 space-y-6">
        <div className="grid gap-6 sm:grid-cols-3">
          ...
          <div><label className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1.5 block">Warehouse Scope</label><SearchableSelect options={[{ value: "", label: "All Warehouses" }, ...warehouses.map((w) => ({ value: w.id, label: w.name }))]} value={warehouseId} onChange={(v) => { setWarehouseId(v); setLines(lines.map((l) => ({ ...l, binId: "" }))); }} placeholder="Select Warehouse" /></div>
        ...
        <div className="overflow-x-auto"><table className="w-full text-sm text-left"><thead className="bg-muted/50 text-xs uppercase text-muted-foreground border-b border-border/50"><tr><th className="px-4 py-3 font-bold w-[35%]">Item</th>{whBins.length > 0 && <th className="px-4 py-3 font-bold w-[20%]">Bin</th>}<th className="px-4 py-3 font-bold w-[20%]">Counted Qty</th><th className="px-4 py-3 font-bold">Notes</th><th className="px-4 py-3 w-10"></th></tr></thead><tbody className="divide-y divide-border/50">
          {lines.map((line) => (
            <tr key={line.id} className="hover:bg-muted/30 transition-colors">
              <td className="p-2"><SearchableSelect options={items.map((i) => ({ value: i.id, label: `${i.name}${i.sku ? ` (${i.sku})` : ""}` }))} value={line.itemId} onChange={(v) => updateLine(line.id, "itemId", v)} placeholder="Select item..." /></td>
              {whBins.length > 0 && <td className="p-2"><SearchableSelect options={[{ value: "", label: "Any" }, ...whBins.map((b: any) => ({ value: b.id, label: b.name }))]} value={line.binId} onChange={(v) => updateLine(line.id, "binId", v)} placeholder="Select bin..." /></td>}
