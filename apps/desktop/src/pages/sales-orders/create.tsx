// apps/desktop/src/pages/sales-orders/create.tsx
...
export default function SalesOrderCreatePage() {
    ...
    React.useEffect(() => {
        const load = async () => {
          try {
            ...
            if (editId) {
                const so = await getSalesOrder(editId);
                setForm({
                    partyId: so.partyId,
                    orderDate: { ad: so.orderDate?.split('T')[0] || "", bs: so.orderDateBs || "" },
                    expectedDelivery: { ad: so.expectedDelivery?.split('T')[0] || "", bs: so.expectedDeliveryBs || "" },
                    customerPoRef: so.customerPoRef || "",
                    memo: so.memo || "",
                    terms: so.terms || "",
                    orderNo: so.orderNo || "NEW",
                });
                setLines(so.items.map((it: any) => ({
                    itemId: it.itemId,
                    qty: String(it.qty),
                    rate: String(it.rate),
                    description: it.description || ""
                })));
                if (so.sundries?.length) {
                    setBillSundries(so.sundries.map((sn: any) => ({
    ...
    const onSave = async () => {
        ...
        try {
            ...
            if (editId) await updateSalesOrder(editId, payload);
    ...
    if (pageLoading) return <div className="p-10 text-center animate-pulse font-black text-slate-300">CALIBRATING REGISTRY...</div>;

    return (
        <div className="space-y-6 animate-fade-in pb-20">
            <div className="flex items-center justify-between">
                <Button variant="ghost" onClick={() => navigate("/sales-orders")} className="rounded-full h-10 px-4 text-slate-500">
                    <ArrowLeft className="mr-2 h-4 w-4" /> Back to List
                </Button>
                <div className="flex gap-3">
                    <Button onClick={onSave} disabled={loading} className="h-11 px-8 rounded-2xl bg-indigo-600 text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-100 italic transition-all active:scale-95">
                        <Save className="mr-2 h-4 w-4" /> Commit Order Manifest
                    </Button>
                </div>
            </div>

            <PageHeader 
                title={editId ? `Calibrate Order: ${form.orderNo}` : "New Sales Order Registry"} 
                description="Synchronize customer demand with your fulfillment pipeline." 
            />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="p-8 bg-white rounded-[32px] border-2 border-slate-50 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="space-y-6">
                            <div className="space-y-2">
                                <div className="flex justify-between items-center">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Consignee Identity</label>
                                    <Button variant="ghost" size="sm" onClick={() => setAddCustomerOpen(true)} className="h-6 text-[9px] font-black text-indigo-600 uppercase">New Customer</Button>
                                </div>
                                <SearchableSelect
                             ...
                             <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Customer PO Reference</label>
                                <Input
                                    value={form.customerPoRef}
                                    onChange={(e) => setForm(f => ({ ...f, customerPoRef: e.target.value }))}
                                    placeholder="External PO identification..."
                        ...
                        <div className="space-y-6">
                            <DualDateInput
                                label="Order Injection Date"
                                value={form.orderDate}
                                onChange={(d) => setForm(f => ({ ...f, orderDate: d }))}
                                accentColor="bg-indigo-600"
                            />
                            <DualDateInput
                                label="Promised Delivery Goal"
                    ...
                    <div className="bg-white rounded-[32px] border-2 border-slate-50 shadow-sm overflow-hidden">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50/50 border-b border-slate-100">
                                <tr>
                                    ...
                                    <th className="px-2 py-4 text-[9px] font-black uppercase tracking-widest text-slate-400">Resource / SKU Profile</th>
                                    <th className="px-2 py-4 text-[9px] font-black uppercase tracking-widest text-slate-400 w-24 text-center">Volume</th>
                                    <th className="px-2 py-4 text-[9px] font-black uppercase tracking-widest text-slate-400 w-32 text-right">Factor</th>
                                ...
                                {lines.map((line, idx) => (
                                    <tr key={idx} className="group hover:bg-slate-50/30 transition-colors">
                                        ...
                                        <td className="px-2 py-4">
                                            <SearchableSelect
                                                valueId={line.itemId}
                                                onChange={(id, opt) => updateLine(idx, { itemId: id, rate: opt?.salesPrice?.toString() || line.rate })}
                                                options={items}
                                                getLabel={i => i.name}
                                        ...
                                        <td className="px-2 py-4">
                                            <Input
                                                ...
                                                className="h-10 rounded-xl text-center font-black border-slate-100 focus:border-indigo-500 transition-all"
                                            />
                                        </td>
                                        <td className="px-2 py-4">
                                            <Input
                                                ...
                                                className="h-10 rounded-xl text-right font-black border-slate-100 focus:border-indigo-500 transition-all"
                                            />
                                        </td>
                        ...
                        <div className="p-6 bg-slate-50/20 border-t border-slate-100 flex justify-between gap-4">
                             <Button variant="outline" onClick={addLine} className="h-12 flex-1 rounded-2xl border-dashed border-slate-200 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-indigo-600 hover:border-indigo-100 hover:bg-indigo-50/30 transition-all">
                                <Plus className="mr-2 h-4 w-4" /> Append Line Item
                             </Button>
                             <Button variant="outline" onClick={() => setAddItemOpen(true)} className="h-12 w-12 rounded-2xl border-2 border-slate-100 text-indigo-600 hover:bg-slate-50">
                ...
                <div className="space-y-8">
                    ...
                    <div className="p-8 bg-slate-900 rounded-[32px] text-white shadow-2xl relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:rotate-12 transition-transform duration-1000"><Calculator className="h-24 w-24 text-indigo-500" /></div>
                        <div className="space-y-5 relative z-10">
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-500 mb-6">
                                <span>Reconciliation Summary</span>
                                <span className="italic">Order Analytics</span>
                            </div>
                            <div className="flex justify-between text-[11px] font-bold uppercase tracking-widest text-slate-400 font-mono">
                                <span>Subtotal</span>
                                    ...
                                    <div key={row.id} className="flex items-center gap-3">
                                        <div className="flex-1 space-y-1">
                                            <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-500">
                                                ...
                                                <span className={row.type === 'less' ? "text-rose-400" : "text-emerald-400 italic"}>
                                                    {row.type === 'less' ? "- " : "+ "}
                                                    <MoneyText value={row.isManual ? parseFloat(row.manualAmount || "0") : (subtotal * parseFloat(row.ratePct || "0")) / 100} />
                                                </span>
                                            </div>
                                        </div>
                                        <button onClick={() => removeSundry(row.id)} className="text-slate-600 hover:text-rose-500 p-1 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="h-3 w-3" /></button>
                                    </div>
                                ))}
                                <Button variant="ghost" size="sm" onClick={() => setAddSundryOpen(true)} className="h-8 w-full border border-dashed border-slate-800 rounded-xl text-[9px] font-black text-slate-500 uppercase hover:bg-white/5">Adjust Reconciliation</Button>
                            </div>

                            <div className="pt-4 flex justify-between items-end">
                                <div className="space-y-1">
                                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400">Net Commit Value</span>
                                    <div className="text-4xl font-black tabular-nums tracking-tighter"><MoneyText value={total} /></div>
                                </div>
                                <div className="h-14 w-14 rounded-2xl bg-indigo-600 flex items-center justify-center shadow-xl shadow-indigo-500/20"><ShoppingBag className="h-8 w-8" /></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-8 bg-white rounded-[32px] border-2 border-slate-50 shadow-sm space-y-6">
                        <div className="space-y-4">
                            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 flex items-center gap-2"><FileText className="h-3 w-3" /> External Narrations (Terms)</label>
                            <textarea
                                value={form.terms}
                                onChange={(e) => setForm(f => ({ ...f, terms: e.target.value }))}
                                placeholder="Terms and conditions for this order..."
                                className="min-h-[100px] w-full rounded-2xl border-2 border-slate-50 bg-slate-50/30 p-4 text-xs outline-none focus:border-indigo-500 focus:bg-white transition-all font-medium resize-none shadow-inner"
                            />
                        </div>
                        <div className="space-y-4">
                             <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 flex items-center gap-2"><Info className="h-3 w-3" /> Auditor Internal memo</label>
                            <textarea
                                value={form.memo}
            ...
            <AddCustomerDialog open={addCustomerOpen} onClose={() => setAddCustomerOpen(false)} onSuccess={(p) => setParties([...parties, p])} />
